...
def apply_sg_filter(df, win=21, poly=3):
    ...
    z = df["CoG_Z_mm"].interpolate().to_numpy()
    ...
    df["CoG_Z_mm_sg"] = savgol_filter(z, window_length=win, polyorder=poly)
...
INPUT_ROOT = BASE_ROOT / "data" / "2_time_series_feature" / "main_research" / "CoG_calib"
...
pattern = re.compile(
    r"detected_(STUDENT|CIPN|NOCIPN)-ROMBERG-(P\d{3})-\d{8}-(EO|EC)-(T[123])-3D_trim_CoG\.csv",
    re.IGNORECASE
)
...
for group in GROUPS:
    ...
    for sid in subjects:
        ...
        for task in TASKS:
            ...
            files_3d = glob.glob(os.path.join(task_dir, "*.csv"))
            ...
            for f in files_3d:
                ...
                if m:
                    g, pid, cond, trial = m.groups()
                    index_3d[(pid, cond, trial)] = f
            ...
            for i, cond in enumerate(CONDITIONS):
                for j, trial in enumerate(TRIALS):
                    ...
                    # --------------------------
                    try:
                        df = apply_sg_filter(pd.read_csv(f3d), win=21, poly=3)
                    ...
                    x = df["CoG_X_mm_sg"].iloc[FRAME_START:n].to_numpy()
                    y = -df["CoG_Z_mm_sg"].iloc[FRAME_START:n].to_numpy()
            ...
            out_path = os.path.join(plot_dir, f"{sid}_{task}_filtered_3D.png")
            plt.tight_layout()
            ...
            summary_path = os.path.join(summary_dir, f"{group}_{task}_summary_3D.csv")
